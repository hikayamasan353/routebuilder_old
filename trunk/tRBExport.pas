...
type
  ...
  TRBExport_Mod= class(TObject)
   protected
    ...
    function CmdFreeObj(distance: double; tracknumber,objid: integer; xoffset,height,angle: double; pitch:double=0; roll:double=0): string;
...
//      RoutePart.add( CmdPitch(ZPosition-25,round((Track.p1.Height-PrevTrack.p2.Height)*40)) );
  ...
  begin
    ...
    begin
      ...
      if (curve<>0)
      or (not (secTrack.special in [csStraight]))
      or (not FormOptions.CatenaryPolesEvery50m)
      or (round(zpos) mod 50 = 0) then
  ...
  {  if xoffs=0 then
    ...
    begin
      if turn then
        RoutePart.Add( CmdFreeObj(zpos,-1,GetFreeobjIndex('walls',secTrack.TSOLeft,'_r.b3d'),xoffs,hoehe+SecTrack.TSOOffsetLeft/1000,winkel,pitch) )
//        RoutePart.Add( CmdFreeObj(zpos+25,-1,GetFreeobjIndex('walls',WallList.getObjectfilenamebyID(secTrack.TSOIDLeft),'_l.b3d'),-xoffs,hoehe+SecTrack.TSOOffsetRight/1000,winkel+180) )
        ...
        RoutePart.Add( CmdFreeObj(zpos,-1,GetFreeobjIndex('walls',secTrack.TSOLeft,'_l.b3d'),xoffs,hoehe+SecTrack.TSOOffsetLeft/1000,winkel,-pitch) );
    end;
...
//        RoutePart.Add( CmdFreeObj(zpos+25,-1,GetFreeobjIndex('walls',WallList.getObjectfilenamebyID(secTrack.TSOIDRight),'_r.b3d'),-xoffs,hoehe+SecTrack.TSOOffsetLeft/1000,winkel+180) )
        RoutePart.Add( CmdFreeObj(zpos,-1,GetFreeobjIndex('walls',secTrack.TSORight,'_l.b3d'),xoffs,hoehe+SecTrack.TSOOffsetRight/1000,winkel,pitch) )
      else
        RoutePart.Add( CmdFreeObj(zpos,-1,GetFreeobjIndex('walls',secTrack.TSORight,'_r.b3d'),xoffs,hoehe+SecTrack.TSOOffsetRight/1000,winkel,-pitch) );
    end;
    // bound freeobj?
...
function TRBExport_Mod.CmdFreeObj(distance: double; tracknumber,objid: integer; xoffset,height,angle,pitch,roll: double): string;
var _angle: double;
...
begin
  ...
  result := floattostrPoint(distance)+', .freeobj ' + inttostr(_tracknumber) + ';'
     + inttostr(objid)+';'+floattostrPoint(xoffset,maxcommadigits) + ';'+ floattostrPoint(height,maxcommadigits)
     + ';' + floattostrpoint(_angle,maxcommadigits)
     + ';' + floattostrpoint(pitch,maxcommadigits)
     + ';' + floattostrpoint(roll,maxcommadigits);
  inc(FObjCount);
end;
